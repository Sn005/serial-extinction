...
**以下のような依頼を受けた場合、自動的にサブエージェント群を使用する：**
...
```
1. /story-analyzer      → 物語分析（既存章との接続確認）
2. /chapter-outliner    → アウトライン作成
3. ユーザーにアウトライン確認を求める（承認待ち）
4. /chapter-writer      → 執筆
5. /proofreader         → 校正
6. /quality-reviewer    → 品質レビュー
7. スコア40未満なら /reviser で修正 → 再レビュー（最大3回）
8. /final-checker       → 最終整合性確認
9. ドキュメント更新（シナリオ構成表、伏線管理等）
```

#### 簡易モード

ユーザーが「簡易モードで」「レビューなしで」等を指定した場合：
- `/story-analyzer` → `/chapter-outliner` → `/chapter-writer` のみ実行
- 品質保証フェーズはスキップ
...
### 🎭 執筆ワークフロー・サブエージェント（推奨）

統合された執筆フローを提供するサブエージェント群です。

| スキル | コマンド | 用途 | フェーズ |
|--------|----------|------|----------|
| **オーケストレーター** | `/writing-orchestrator` | 全フローを統括・自動実行 | 全体 |
| **ストーリーアナライザー** | `/story-analyzer` | 物語の流れ・構成を分析 | 準備 |
| **チャプターアウトライナー** | `/chapter-outliner` | 章の詳細アウトラインを作成 | 準備 |
| **チャプターライター** | `/chapter-writer` | 章を執筆 | 執筆 |
| **プルーフリーダー** | `/proofreader` | 校正（誤字・表記ゆれ） | 品質保証 |
| **クオリティレビュアー** | `/quality-reviewer` | 総合的な品質レビュー | 品質保証 |
| **リバイザー** | `/reviser` | レビューに基づく修正 | 品質保証 |
| **ファイナルチェッカー** | `/final-checker` | 全体整合性の最終確認 | 最終確認 |
...
### 🚀 推奨：統合ワークフロー（オーケストレーター使用）

```
/writing-orchestrator Extinction XX
```

これにより以下のフローが自動実行されます：

```
Phase 1: 準備
  /story-analyzer → /chapter-outliner → ユーザー承認

Phase 2: 執筆
  /chapter-writer

Phase 3: 品質保証
  /proofreader → /quality-reviewer → (/reviser → 再レビュー) × 必要回数
